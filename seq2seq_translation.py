...
def readLangs(lang1, lang2, reverse=False):
    ...
    for filename in datasets:
        with open(filename, encoding='utf-8') as file:
            ...
            pairs = [[normalize_string(s) for s in row] for row in reader if len(row) == 2]
            all_pairs.extend(pairs)

    if reverse:
        all_pairs = [list(reversed(p)) for p in all_pairs]
...
if __name__ == '__main__':
    input_lang, output_lang, pairs = prepareData('eng', 'yor', True)
...
evaluateAndShowAttention('e nle sa ! mo mo yin')
